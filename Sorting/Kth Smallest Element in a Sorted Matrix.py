...
# the total time complexity for this algorithm comes down to be O(X+Klog(X)) where X is min(K, N). N: len(matrix), the number of sorted lists
...
# It's important to know what row and column an element belongs to. Without knowing that, we won't be able to move forward in that particular list. So, apart from adding an element to the heap, we also need to add its row and column number.

# Say the minimum triplet in the heap is (v, r, c), then when it is pop out, which triplet to add to the heap?
#   - The next triplet doesn't HAVE to be the next minium element in matrix. It shall be the next POSSIBLE minium element
...
class Solution:
    def kthSmallest(self, matrix: List[List[int]], k: int) -> int:
        min_heap = []  # Its size does not to be k. Our goal is not to KEEP k smallest elements in it. But to POP smallest element K times.
        ...
        # while havn't poped smallest element k times
        while k > 0:
