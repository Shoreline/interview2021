...
#  * 3) convert int into double while computing slope (may have precision issue. But works for the x,y range in problem
#       statement)
#  */

# points: [[x1, y1], [x2, y2], ...]
# helper(currentPoint, points) returns the maximum number of points for all lines passing currentPoint
#   It computes the slops of lines formed by the currentPoint and each point in points.
...
class Solution:
    def maxPoints(self, points: List[List[int]]) -> int:
        def helper(currentPoint, points):
            ...
            return max_count + 1 + duplicates  # "1" is this currentPoint itself

        res = 0
        # avoid duplicated checking
        # If there are max 3 points p1, p2, p3 on one line, then max_points_on_same_line_including_point(a_point) is the
        # same for p1, p2, p3, so only need to run this function once for p1. And ok to exclude p1 for further
        # computation
        while points:
