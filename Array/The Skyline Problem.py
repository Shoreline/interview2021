...
class Solution:
    def getSkyline(self, buildings: List[List[int]]) -> List[List[int]]:
        ...
        # heights_heap 记录了对于一条高为 h 的 skyline, 他将持续到什么 position 才结束: [h, endposition]
        # 在同时有多条 skyline 的时候, h 最高的那条 skyline 会掩盖 h 低的 skyline, 因此在 event 触发时, 需要得到当前最高的 skyline
        # 所以利用 heap 结构存储 heights_heap, 它的第一个值永远为列表中的最小值: 因此在 event 中记录的是 -h, heap 结构就会返回最高的 skyline. 同时, h 必须在 endposition 之前, 因为它按第一个元素排序
        # Remember to set initial elements in res and heights_heap
        res = [[0, 0]]  # res[-1] is the current skyline height
        heights_heap = [(0, float('inf'))]  # a heap to save effective heights: (height, end_pos)

        for l, neg_h, r in events:
            # 触发 event 时, 首先要做的就是清除已经到 endposition 的 skyline
            # heights_heap: [h, endposition]
            # 如果当前 position 大于等于了 heights_heap 中的 endposition, 那么该 skyline 会被清除掉
            # 由于在有 high skyline 的情况下, low skyline 不会有影响, 因此, 只需要按从高到低的方式清除 skyline, 直到剩下一个最高的 skyline 并且它的 endposition 大于当前 position

            # Clean outdated skylines first.
            # Note that only the highest height matters in forming skyline, so no need to throughly clean all heights.
            while heights_heap[0][1] <= l:
            ...
            if res[-1][1] != -heights_heap[0][0]:
                res.append([l, -heights_heap[0][0]])  # current event starts from l
