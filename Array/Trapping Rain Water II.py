...
# BFS with min heap
# Keep expending the "edge cells". Start with the initial edge cells (the most outer circle cells), then expend
# inwards.
#   level: water level
#   Each time, expand from the lowest edge cell: -> any unvisited neighboring cell with lower height than cur_level
#   (not the lowest edge cell height!) can trap water
class Solution:
        ...
        for i in range(m):
            for j in range(n):
                if i == 0 or i == m - 1 or j == 0 or j == n - 1:
                    heapq.heappush(heap, (heightMap[i][j], i, j)) # a tuple of (height, i, j)
                    heightMap[i][j] = -1  # Use -1 to label visited cells

        # Start from level 0
        level, res = 0, 0
        while heap:
            height, x, y = heapq.heappop(heap)  # get the edge cell with the lowest height
            level = max(height, level) # water level can preserve during looking inward
